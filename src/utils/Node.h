...
namespace util {
    struct Node {
        explicit Node(int idd, double xx, double yy, double c =0.0, int pid = -1) : id(idd),x(xx), y(yy), cost(c), parent(pid) {}
    ...
    struct compare_cost{
        bool operator()(const util::Node& p1, const util::Node& p2) {
            if (p1.cost + p1.hCost > p2.cost + p2.hCost) return true;
            else if (p1.cost + p1.hCost == p2.cost + p2.hCost && p1.hCost >= p2.hCost) return true;
            return false;
        }
    };
}
