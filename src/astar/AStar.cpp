...
namespace astar {
    ...
    AStar::AStar(std::shared_ptr<util::GridMap<unsigned char>> map, HeuristicType heuristic) : IPlanner(map),
                                                                                               currentHeuristic_(
                                                                                                       heuristic),
                                                                                               goalIndex_(-1),
                                                                                               startIndex_(-1) {
    ...
    std::vector<util::Point> AStar::makePlan(const util::Point &start, const util::Point &goal) {
        ...
        std::vector<double> gCosts(map_->getCellWidth() * map_->getCellHeight(),
        ...
        while (!priorityCosts.empty() && cameFrom[static_cast<unsigned long>(goalIndex_)] == -1) {
            ...
            for (unsigned i = 0; i < neighborIndexes.size(); i++) {
                auto costFromHere = gCosts[currentNode.index] + getMoveCost(currentNode.index, neighborIndexes[i]);
                if (cameFrom[static_cast<unsigned long>(neighborIndexes[i])] == -1) {
                    gCosts[static_cast<unsigned long>(neighborIndexes[i])] = costFromHere;
                    Node nextNode;
        ...
        while (currentNode.index != startIndex_) {
            auto loc = map_->indexToMap(
                    static_cast<unsigned int>(cameFrom[static_cast<unsigned long>(currentNode.index)]));
